# 购房决策智能助手 - 技术实现方案

## 概述

本文档定义了购房决策智能助手的具体技术实现方案。

**对外表述**: 多智能体协作系统  
**内部实现**: 伪多Agent（单Agent + 多角色 + 多工具），手搓实现  
**不使用LangChain的理由**: 需求不复杂，手搓更可控、更易调试、代码更清晰

---

## 一、技术栈

### 后端

| 模块 | 技术 | 版本 | 说明 |
|------|------|------|------|
| Web框架 | FastAPI | 0.109+ | 异步支持，自动文档 |
| ASGI服务器 | Uvicorn | 0.27+ | 高性能 |
| HTTP客户端 | httpx | 0.26+ | 异步调用LLM API |
| 向量库 | Chroma | 0.4+ | RAG检索 |
| Embedding | sentence-transformers | - | 本地Embedding模型 |
| 数据库 | PostgreSQL | 15+ | 稳定可靠，JSON支持好 |
| 缓存 | Redis | 7+ | 会话管理、热点缓存 |

### 前端

| 模块 | 技术 | 说明 |
|------|------|------|
| 框架 | Vue.js 3 | 组合式API |
| 构建 | Vite | 快速开发 |
| UI组件 | Element Plus | 成熟稳定 |
| HTTP | Axios | 请求封装 |
| 图表 | ECharts | 走势图展示 |

### LLM

| 模型 | 用途 | 价格 |
|------|------|------|
| DeepSeek-V3 | 主力模型 | ￥1/百万token |
| DeepSeek-R1 | 复杂推理（可选） | ￥4/百万token |

---

## 二、项目结构

```
house-purchase-assistant/
├── backend/
│   ├── app/
│   │   ├── __init__.py
│   │   ├── main.py              # FastAPI入口
│   │   ├── config.py            # 配置管理
│   │   ├── api/
│   │   │   ├── __init__.py
│   │   │   ├── auth.py          # 用户认证接口
│   │   │   ├── chat.py          # 对话接口
│   │   │   ├── calculator.py    # 计算器接口
│   │   │   └── conversation.py  # 对话历史接口
│   │   ├── agent/
│   │   │   ├── __init__.py
│   │   │   ├── engine.py        # Agent引擎核心
│   │   │   ├── roles.py         # 角色定义
│   │   │   ├── intent.py        # 意图识别
│   │   │   └── tools/
│   │   │       ├── __init__.py
│   │   │       ├── base.py      # 工具基类
│   │   │       ├── financial.py # 财务计算工具
│   │   │       ├── market.py    # 市场查询工具
│   │   │       ├── policy.py    # 政策检索工具
│   │   │       └── news.py      # 新闻搜索工具
│   │   ├── llm/
│   │   │   ├── __init__.py
│   │   │   └── client.py        # LLM客户端封装
│   │   ├── rag/
│   │   │   ├── __init__.py
│   │   │   ├── embeddings.py    # Embedding服务
│   │   │   └── retriever.py     # 检索服务
│   │   ├── models/
│   │   │   ├── __init__.py
│   │   │   └── schemas.py       # Pydantic模型
│   │   ├── services/
│   │   │   ├── __init__.py
│   │   │   └── conversation.py  # 对话历史服务
│   │   └── utils/
│   │       └── __init__.py
│   ├── data/                    # 数据文件（从根目录链接）
│   ├── requirements.txt
│   └── Dockerfile
├── frontend/
│   ├── src/
│   │   ├── main.js
│   │   ├── App.vue
│   │   ├── views/
│   │   │   ├── Chat.vue         # 对话页面
│   │   │   ├── Calculator.vue   # 计算器页面
│   │   │   └── Market.vue       # 市场分析页面
│   │   ├── components/
│   │   │   ├── ChatMessage.vue
│   │   │   ├── RoleTag.vue
│   │   │   └── PriceChart.vue
│   │   ├── api/
│   │   │   └── index.js
│   │   └── utils/
│   ├── package.json
│   └── vite.config.js
├── data/                        # 数据文件
│   ├── config/
│   ├── market/
│   └── knowledge/
├── docs/                        # 文档
└── docker-compose.yml
```


---

## 三、核心模块实现

### 3.1 LLM客户端封装

```python
# app/llm/client.py
import httpx
from typing import Optional

class DeepSeekClient:
    """DeepSeek API 客户端"""
    
    def __init__(self, api_key: str, base_url: str = "https://api.deepseek.com"):
        self.api_key = api_key
        self.base_url = base_url
        self.client = httpx.AsyncClient(timeout=60.0)
    
    async def chat(
        self,
        messages: list[dict],
        model: str = "deepseek-chat",
        temperature: float = 0.7,
        tools: Optional[list[dict]] = None
    ) -> dict:
        """调用对话接口"""
        headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json"
        }
        
        payload = {
            "model": model,
            "messages": messages,
            "temperature": temperature
        }
        
        if tools:
            payload["tools"] = tools
            payload["tool_choice"] = "auto"
        
        response = await self.client.post(
            f"{self.base_url}/v1/chat/completions",
            headers=headers,
            json=payload
        )
        response.raise_for_status()
        return response.json()
    
    async def chat_stream(self, messages: list[dict], **kwargs):
        """流式输出"""
        kwargs["stream"] = True
        # 实现流式响应...
```

### 3.2 工具定义框架

```python
# app/agent/tools/base.py
from abc import ABC, abstractmethod
from typing import Any
from pydantic import BaseModel, Field

class ToolParameter(BaseModel):
    """工具参数定义"""
    name: str = Field(..., description="参数名称")
    type: str = Field(..., description="参数类型: string, number, integer, boolean")
    description: str = Field(..., description="参数描述")
    required: bool = Field(default=True, description="是否必需")
    enum: list[str] | None = Field(default=None, description="枚举值列表")

    def to_json_schema(self) -> dict:
        """转换为 JSON Schema 格式"""
        schema = {
            "type": self.type,
            "description": self.description
        }
        if self.enum:
            schema["enum"] = self.enum
        return schema

class BaseTool(ABC):
    """工具基类，所有工具必须继承此类"""
    name: str  # 工具名称
    description: str  # 工具描述
    parameters: list[ToolParameter]  # 参数列表
    
    @abstractmethod
    async def execute(self, **kwargs) -> dict:
        """执行工具"""
        pass
    
    def validate_params(self, **kwargs) -> None:
        """
        验证输入参数
        
        Raises:
            ValueError: 参数验证失败时抛出
        """
        for param in self.parameters:
            if param.required and param.name not in kwargs:
                raise ValueError(f"缺少必需参数: {param.name}")
            if param.name in kwargs:
                value = kwargs[param.name]
                # 检查枚举值
                if param.enum and value not in param.enum:
                    raise ValueError(f"参数 {param.name} 值无效: {value}")
    
    def to_openai_schema(self) -> dict:
        """转换为OpenAI工具格式"""
        properties = {}
        required = []
        for param in self.parameters:
            properties[param.name] = param.to_json_schema()
            if param.required:
                required.append(param.name)
        return {
            "type": "function",
            "function": {
                "name": self.name,
                "description": self.description,
                "parameters": {
                    "type": "object",
                    "properties": properties,
                    "required": required
                }
            }
        }
```

### 3.3 财务计算工具示例

```python
# app/agent/tools/financial.py
from .base import BaseTool, ToolParameter

class CalcLoanTool(BaseTool):
    """贷款计算工具"""
    name = "calc_loan"
    description = "计算贷款方案，包括月供、总利息等"
    parameters = [
        ToolParameter(name="price", type="number", description="房屋总价（元）"),
        ToolParameter(name="down_payment_ratio", type="number", description="首付比例，如0.3表示30%"),
        ToolParameter(name="years", type="integer", description="贷款年限"),
        ToolParameter(name="rate", type="number", description="年利率，如3.6表示3.6%"),
        ToolParameter(name="method", type="string", description="还款方式：equal_payment(等额本息) 或 equal_principal(等额本金)", required=False)
    ]
    
    async def execute(
        self,
        price: float,
        down_payment_ratio: float,
        years: int,
        rate: float,
        method: str = "equal_payment"
    ) -> dict:
        down_payment = price * down_payment_ratio
        loan_amount = price - down_payment
        monthly_rate = rate / 100 / 12
        months = years * 12
        
        if method == "equal_payment":
            # 等额本息
            if monthly_rate > 0:
                monthly_payment = loan_amount * monthly_rate * (1 + monthly_rate)**months / ((1 + monthly_rate)**months - 1)
            else:
                monthly_payment = loan_amount / months
            total_payment = monthly_payment * months
            total_interest = total_payment - loan_amount
        else:
            # 等额本金
            principal_per_month = loan_amount / months
            first_month_interest = loan_amount * monthly_rate
            first_month_payment = principal_per_month + first_month_interest
            total_interest = (loan_amount * monthly_rate * (months + 1)) / 2
            monthly_payment = first_month_payment  # 首月
            total_payment = loan_amount + total_interest
        
        return {
            "down_payment": round(down_payment, 2),
            "loan_amount": round(loan_amount, 2),
            "monthly_payment": round(monthly_payment, 2),
            "total_payment": round(total_payment, 2),
            "total_interest": round(total_interest, 2),
            "years": years,
            "rate": rate,
            "method": "等额本息" if method == "equal_payment" else "等额本金"
        }
```


### 3.4 角色定义

```python
# app/agent/roles.py
from dataclasses import dataclass

@dataclass
class Role:
    """角色定义"""
    id: str
    name: str
    icon: str
    description: str
    system_prompt: str
    tools: list[str]
    trigger_keywords: list[str]

ROLES = {
    "financial_advisor": Role(
        id="financial_advisor",
        name="财务顾问",
        icon="💰",
        description="专业的购房财务顾问",
        system_prompt="""你是专业的购房财务顾问，具备以下能力：
- 精通公积金贷款、商业贷款、组合贷款的计算
- 熟悉各类购房税费（契税、增值税、个税、中介费等）
- 能够评估用户的还款压力和购房能力
- 给出专业、准确、易懂的财务建议

回答要求：
1. 数据准确，给出具体数字
2. 解释计算过程
3. 评估风险和压力
4. 给出可行建议""",
        tools=["calc_loan", "calc_tax", "calc_total_cost", "assess_pressure"],
        trigger_keywords=["贷款", "月供", "首付", "税费", "能买吗", "买得起", "压力", "成本", "利息"]
    ),
    
    "policy_expert": Role(
        id="policy_expert",
        name="政策专家",
        icon="📋",
        description="购房政策专家",
        system_prompt="""你是购房政策专家，具备以下能力：
- 熟悉南宁、柳州的限购限贷政策
- 了解公积金贷款政策和申请条件
- 掌握购房流程和所需材料
- 能解答各类购房政策问题

回答要求：
1. 准确引用政策内容
2. 说明政策来源和时效
3. 解释适用条件
4. 提醒注意事项""",
        tools=["search_policy", "search_faq", "search_guide"],
        trigger_keywords=["限购", "政策", "资格", "条件", "公积金", "流程", "材料", "手续", "规定"]
    ),
    
    "market_analyst": Role(
        id="market_analyst",
        name="市场分析师",
        icon="📈",
        description="房地产市场分析师",
        system_prompt="""你是房地产市场分析师，具备以下能力：
- 分析房价走势和市场趋势
- 对比不同区域的房价和发展潜力
- 分析供需关系和去化周期
- 判断购房时机并给出建议

回答要求：
1. 数据驱动，引用具体数据
2. 客观分析，不夸大不贬低
3. 给出趋势判断
4. 说明判断依据""",
        tools=["query_market", "query_price_trend", "compare_districts", "judge_timing", "search_news"],
        trigger_keywords=["房价", "走势", "涨", "跌", "时机", "区域", "对比", "市场", "行情", "趋势"]
    ),
    
    "purchase_consultant": Role(
        id="purchase_consultant",
        name="购房顾问",
        icon="✅",
        description="综合购房顾问",
        system_prompt="""你是综合购房顾问，具备以下能力：
- 整合财务、政策、市场等多方面信息
- 给出综合性的购房建议
- 提示潜在风险
- 规划下一步行动

回答要求：
1. 综合各方面信息
2. 建议具体可行
3. 风险提示到位
4. 给出行动清单""",
        tools=["generate_report"],
        trigger_keywords=["建议", "怎么办", "下一步", "综合", "报告", "总结"]
    )
}
```

### 3.5 意图识别

```python
# app/agent/intent.py
"""
意图识别模块 - DAG 执行计划生成

核心改进：
1. 去掉关键词匹配，完全由 LLM 识别意图
2. LLM 返回 DAG 格式的执行计划，包含角色和依赖关系
3. 支持循环依赖检测
"""
from dataclasses import dataclass, field

@dataclass
class ExecutionNode:
    """
    执行节点
    
    Attributes:
        role_id: 角色 ID
        depends_on: 依赖的角色 ID 列表（空表示无依赖，可立即执行）
    """
    role_id: str
    depends_on: list[str] = field(default_factory=list)

@dataclass
class ExecutionPlan:
    """
    DAG 执行计划
    
    Attributes:
        nodes: 执行节点列表
        reason: LLM 给出的规划理由
    """
    nodes: list[ExecutionNode] = field(default_factory=list)
    reason: str = ""
    
    def get_ready_nodes(self, completed: set[str]) -> list[ExecutionNode]:
        """获取当前可执行的节点（依赖都已完成）"""
        ready = []
        for node in self.nodes:
            if node.role_id in completed:
                continue
            if all(dep in completed for dep in node.depends_on):
                ready.append(node)
        return ready

class IntentRecognizer:
    """意图识别器 - 生成 DAG 执行计划"""
    
    # LLM 规划提示词（核心）
    SYSTEM_PROMPT = """你是一个智能任务规划器，负责分析用户的购房咨询问题，
决定需要哪些专家角色来回答，以及它们之间的执行依赖关系。

## 可用角色
1. financial_advisor（财务顾问）- 贷款计算、税费估算、成本分析
2. policy_expert（政策专家）- 限购限贷、公积金政策、购房资格
3. market_analyst（市场分析师）- 房价走势、区域对比、时机判断
4. purchase_consultant（购房顾问）- 综合建议、整合多方信息

## 依赖关系判断规则
1. 无依赖（可并行）：两个角色的分析相互独立
2. 有依赖（需串行）：一个角色需要另一个角色的结果
3. 整合场景：purchase_consultant 应该依赖所有其他参与的专家

## 输出格式
```json
{
  "nodes": [
    {"role_id": "角色ID", "depends_on": ["依赖的角色ID"]}
  ],
  "reason": "简要说明规划理由"
}
```"""
    
    async def plan_execution(self, user_input: str) -> ExecutionPlan:
        """分析用户输入，生成 DAG 执行计划"""
        response = await self.llm.chat([
            {"role": "system", "content": self.SYSTEM_PROMPT},
            {"role": "user", "content": f"用户问题：{user_input}"}
        ], temperature=0.1)
        
        # 解析 JSON 响应，验证 DAG 有效性
        plan = self._parse_plan_response(response)
        
        # 检测循环依赖
        if not self._validate_dag(plan.nodes):
            # 清除所有依赖，降级为并行执行
            for node in plan.nodes:
                node.depends_on = []
        
        return plan
    
    def _validate_dag(self, nodes: list[ExecutionNode]) -> bool:
        """使用拓扑排序检测循环依赖"""
        # 构建入度表，执行拓扑排序
        # 如果访问节点数 < 总节点数，存在循环
        ...
```


### 3.6 Agent引擎核心（DAG 调度）

```python
# app/agent/engine.py
"""
Agent 引擎核心 - DAG 调度实现

核心改进：
1. 使用 DAG 调度替代简单串行执行
2. 支持并行执行无依赖的角色
3. 使用 asyncio.gather() 实现真正的并行
"""
import asyncio
from dataclasses import dataclass
from typing import AsyncGenerator
from .intent import ExecutionPlan, ExecutionNode, get_role

@dataclass
class RoleResult:
    """角色执行结果"""
    role_id: str
    role_name: str
    content: str
    success: bool = True

class AgentEngine:
    """伪多Agent引擎 - DAG 调度版本"""
    
    async def process(self, user_input: str, session_id: str, mode: str = "standard"):
        """处理用户输入，流式返回结果"""
        
        # 1. 意图识别，生成 DAG 执行计划
        plan = await self.intent_recognizer.plan_execution(user_input)
        
        # 2. DAG 调度执行
        if mode == "discussion" and len(plan.nodes) > 1:
            async for event in self._process_discussion_mode(user_input, plan, context):
                yield event
        else:
            async for event in self._process_dag_mode(user_input, plan, context):
                yield event
        
        yield {"type": "done"}
    
    async def _process_dag_mode(
        self, user_input: str, plan: ExecutionPlan, context
    ) -> AsyncGenerator[dict, None]:
        """
        DAG 调度模式：根据依赖关系并行/串行执行角色
        
        算法：
        1. 获取当前可执行节点（依赖都已完成）
        2. 单节点：直接执行
        3. 多节点：asyncio.gather() 并行执行
        4. 循环直到所有节点完成
        """
        completed: set[str] = set()  # 已完成的角色 ID
        
        while True:
            # 获取当前可执行的节点
            ready_nodes = plan.get_ready_nodes(completed)
            
            if not ready_nodes:
                break  # 没有可执行的节点了
            
            if len(ready_nodes) == 1:
                # 单节点，直接执行
                node = ready_nodes[0]
                async for event in self._execute_node(node, user_input, context):
                    yield event
                completed.add(node.role_id)
            else:
                # 多节点，并行执行
                # 先发送所有 role_start 事件
                for node in ready_nodes:
                    role = get_role(node.role_id)
                    yield {"type": "role_start", "role": role.id, "name": role.name, "icon": role.icon}
                
                # 并行执行所有角色
                tasks = [self._execute_role_async(node, user_input, context) for node in ready_nodes]
                results = await asyncio.gather(*tasks, return_exceptions=True)
                
                # 按顺序发送结果
                for node, result in zip(ready_nodes, results):
                    if isinstance(result, Exception):
                        yield {"type": "role_result", "role": node.role_id, "content": f"执行出错: {result}"}
                    else:
                        context.role_results[node.role_id] = result.content
                        yield {"type": "role_result", "role": result.role_id, "content": result.content}
                    completed.add(node.role_id)
    
    async def _execute_role_async(self, node: ExecutionNode, user_input: str, context) -> RoleResult:
        """异步执行角色（用于并行执行）"""
        role = get_role(node.role_id)
        content = await self._execute_role(role, user_input, context)
        return RoleResult(role_id=role.id, role_name=role.name, content=content)
```

### 3.7 高级模式（Discussion Mode）

高级模式保持原有的多角色对话协商逻辑：

```python
    async def _process_discussion_mode(
        self, 
        user_input: str, 
        context: dict
    ) -> AsyncGenerator[dict, None]:
        """高级模式：多角色对话协商"""
        
        # 1. 意图识别，确定参与讨论的角色
        execution_plan = await self.intent_recognizer.plan_execution(user_input)
        participating_roles = execution_plan.roles
        
        # 2. 多轮对话协商
        discussion_history = []
        max_rounds = 5  # 最大讨论轮数
        
        for round_num in range(max_rounds):
            for role in participating_roles:
                # 通知前端当前发言角色
                yield {"type": "role_start", "role": role.id, "name": role.name, "icon": role.icon}
                
                # 角色发言（基于讨论历史）
                response = await self._role_discuss(role, user_input, discussion_history)
                discussion_history.append({"role": role.name, "content": response})
                
                # 推送讨论内容
                yield {
                    "type": "discussion", 
                    "from": role.id, 
                    "name": role.name,
                    "icon": role.icon,
                    "content": response,
                    "round": round_num + 1
                }
            
            # 检查是否达成共识或信息充足
            if await self._check_consensus(discussion_history, user_input):
                break
        
        # 3. 购房顾问总结讨论结果
        yield {"type": "role_start", "role": "consultant", "name": "购房顾问", "icon": "✅"}
        summary = await self._summarize_discussion(discussion_history, user_input)
        yield {"type": "role_result", "role": "consultant", "content": summary}
        
        yield {"type": "done"}
    
    async def _execute_role(self, role: Role, user_input: str, context: dict) -> str:
        """执行单个角色的分析"""
        
        # 构建消息
        messages = [
            {"role": "system", "content": role.system_prompt},
            {"role": "user", "content": user_input}
        ]
        
        # 添加上下文
        if context.get("history"):
            messages = [messages[0]] + context["history"] + [messages[-1]]
        
        # 获取角色可用的工具
        available_tools = [
            self.tools[t].to_openai_schema() 
            for t in role.tools 
            if t in self.tools
        ]
        
        # 调用LLM
        response = await self.llm.chat(messages, tools=available_tools if available_tools else None)
        
        # 处理工具调用
        message = response["choices"][0]["message"]
        if message.get("tool_calls"):
            # 执行工具
            tool_results = await self._execute_tools(message["tool_calls"])
            
            # 将工具结果加入对话，再次调用LLM生成最终回答
            messages.append(message)
            for tool_call, result in tool_results:
                messages.append({
                    "role": "tool",
                    "tool_call_id": tool_call["id"],
                    "content": str(result)
                })
            
            final_response = await self.llm.chat(messages)
            return final_response["choices"][0]["message"]["content"]
        
        return message["content"]
    
    async def _execute_tools(self, tool_calls: list) -> list:
        """执行工具调用"""
        results = []
        for tool_call in tool_calls:
            func_name = tool_call["function"]["name"]
            func_args = json.loads(tool_call["function"]["arguments"])
            
            if func_name in self.tools:
                result = await self.tools[func_name].execute(**func_args)
                results.append((tool_call, result))
        
        return results
    
    async def _synthesize(self, role_results: dict, user_input: str) -> str:
        """整合多角色结果"""
        synthesis_prompt = f"""基于以下各专家的分析，给出综合购房建议：

用户问题：{user_input}

各专家分析：
{chr(10).join([f"【{ROLES[r].name}】{c}" for r, c in role_results.items()])}

请综合以上信息，给出：
1. 核心结论
2. 综合建议
3. 风险提示
4. 下一步行动"""
        
        response = await self.llm.chat([
            {"role": "system", "content": ROLES["purchase_consultant"].system_prompt},
            {"role": "user", "content": synthesis_prompt}
        ])
        
        return response["choices"][0]["message"]["content"]
```

### 3.7 RAG检索实现

实际实现采用 Chroma HTTP 客户端连接远程服务，使用本地 BGE 中文模型生成向量。

**架构特点**：
- 使用 `HttpClient` 连接 Docker 中的 Chroma 服务（端口 8001）
- 使用本地 BGE 模型（`bge-base-zh-v1.5`）生成中文向量，效果更好
- 支持降级：Chroma 连接失败时自动切换到关键词匹配

```python
# app/db/chroma.py
import chromadb
from chromadb.config import Settings as ChromaSettings
from chromadb.utils import embedding_functions
from pathlib import Path

class ChromaClient:
    """Chroma 向量数据库客户端"""
    
    # Collection 名称常量
    COLLECTION_POLICIES = "policies"  # 限购限贷政策
    COLLECTION_FAQ = "faq"            # 常见问题
    COLLECTION_GUIDES = "guides"      # 购房指南
    
    def __init__(
        self, 
        host: str = "localhost", 
        port: int = 8001,
        embedding_model_path: str = None  # 如 ~/models/bge-base-zh-v1.5
    ):
        self._host = host
        self._port = port
        self._client = None
        self._embedding_fn = None
        
        # 设置本地 BGE embedding 模型
        if embedding_model_path:
            model_path = Path(embedding_model_path).expanduser()
            if model_path.exists():
                self._embedding_fn = embedding_functions.SentenceTransformerEmbeddingFunction(
                    model_name=str(model_path)
                )
        
        # 降级到默认模型
        if self._embedding_fn is None:
            self._embedding_fn = embedding_functions.DefaultEmbeddingFunction()
    
    async def connect(self) -> None:
        """连接 Chroma 服务"""
        self._client = chromadb.HttpClient(
            host=self._host,
            port=self._port,
            settings=ChromaSettings(anonymized_telemetry=False)
        )
        self._client.heartbeat()  # 测试连接
    
    async def disconnect(self) -> None:
        """断开连接"""
        self._client = None
    
    def query(
        self,
        collection_name: str,
        query_text: str,
        n_results: int = 5,
        where: dict = None
    ) -> dict:
        """查询相似文档"""
        collection = self._client.get_or_create_collection(
            name=collection_name,
            embedding_function=self._embedding_fn
        )
        
        query_params = {
            "query_texts": [query_text],
            "n_results": n_results
        }
        if where:
            query_params["where"] = where
        
        return collection.query(**query_params)
```

**应用启动时初始化**（`app/main.py`）：

```python
@asynccontextmanager
async def lifespan(app: FastAPI):
    # 初始化 Chroma 向量数据库
    from app.db import chroma
    try:
        chroma.chroma_client = chroma.ChromaClient(
            host="localhost",
            port=8001,
            embedding_model_path=settings.EMBEDDING_MODEL_PATH
        )
        await chroma.chroma_client.connect()
        logger.info("Chroma 向量数据库连接成功")
    except Exception as e:
        logger.warning(f"Chroma 连接失败，将使用关键词匹配: {e}")
        chroma.chroma_client = None
    
    yield
    
    # 关闭时断开连接
    if chroma.chroma_client:
        await chroma.chroma_client.disconnect()
```

**降级策略**：政策检索工具（`app/agent/tools/policy.py`）在 Chroma 不可用时自动使用关键词匹配。


---

## 四、API接口设计

### 4.1 对话接口

```python
# app/api/chat.py
from fastapi import APIRouter, HTTPException, Depends
from fastapi.responses import StreamingResponse
from pydantic import BaseModel, Field
from typing import Optional

router = APIRouter()

class ChatRequest(BaseModel):
    session_id: str = Field(..., description="会话 ID")
    message: str = Field(..., description="用户消息")
    mode: str = Field(default="standard", description="执行模式: standard 或 discussion")
    conversation_id: Optional[str] = Field(default=None, description="对话ID（用于持久化）")
    user_id: Optional[str] = Field(default=None, description="用户ID（可选）")

@router.post("/chat")
async def chat(request: ChatRequest):
    """
    对话接口（流式响应）
    
    返回 SSE 事件类型：
    - conversation_created: 新对话创建，包含 conversation_id
    - thinking_start: 思考开始（AI 开始分析问题）
    - thinking_step: 思考步骤，包含 step_type 和 content
      - step_type: "planning" - 意图识别和规划阶段
      - step_type: "role_dispatch" - 角色调度阶段，展示哪个角色正在分析
    - role_start: 角色开始处理
    - tool_call: 工具调用事件，包含 tool_name、tool_args、role
    - tool_result: 工具执行结果，包含 tool_name、content、role
    - role_result: 角色处理结果
    - discussion: 讨论事件（高级模式）
    - error: 错误事件
    - done: 处理完成
    """
    
    async def generate():
        async for chunk in agent_engine.process(
            user_input=request.message,
            context=get_session_context(request.session_id),
            mode=request.mode
        ):
            yield f"data: {json.dumps(chunk, ensure_ascii=False)}\n\n"
    
    return StreamingResponse(
        generate(),
        media_type="text/event-stream"
    )

@router.post("/chat/sync")
async def chat_sync(request: ChatRequest):
    """对话接口（同步响应，用于测试）"""
    results = []
    async for chunk in agent_engine.process(
        user_input=request.message,
        context=get_session_context(request.session_id)
    ):
        results.append(chunk)
    return {"results": results}
```

### 4.2 计算器接口

```python
# app/api/calculator.py
from fastapi import APIRouter
from pydantic import BaseModel

router = APIRouter()

class LoanCalcRequest(BaseModel):
    price: float           # 房价
    down_payment_ratio: float  # 首付比例
    years: int             # 贷款年限
    rate: float            # 利率
    method: str = "equal_payment"  # 还款方式

class TaxCalcRequest(BaseModel):
    price: float           # 房价
    area: float            # 面积
    is_first_home: bool    # 是否首套
    house_age_years: int   # 房龄

@router.post("/calc/loan")
async def calc_loan(request: LoanCalcRequest):
    """贷款计算"""
    tool = TOOL_REGISTRY["calc_loan"]
    result = await tool.execute(**request.dict())
    return result

@router.post("/calc/tax")
async def calc_tax(request: TaxCalcRequest):
    """税费计算"""
    tool = TOOL_REGISTRY["calc_tax"]
    result = await tool.execute(**request.dict())
    return result

@router.post("/calc/total_cost")
async def calc_total_cost(request: dict):
    """总成本计算"""
    tool = TOOL_REGISTRY["calc_total_cost"]
    result = await tool.execute(**request)
    return result
```

### 4.3 市场数据接口

```python
# app/api/market.py
from fastapi import APIRouter

router = APIRouter()

@router.get("/market/{city}")
async def get_market_data(city: str):
    """获取城市市场数据"""
    data = load_market_data(city)
    return data

@router.get("/market/{city}/districts")
async def get_district_data(city: str):
    """获取各区数据"""
    data = load_district_data(city)
    return data

@router.get("/market/{city}/trend")
async def get_price_trend(city: str, district: str = None):
    """获取房价走势"""
    data = load_price_trend(city, district)
    return data
```

### 4.4 对话历史接口

```python
# app/api/conversation.py
from fastapi import APIRouter

router = APIRouter(prefix="/conversations")

@router.get("")
async def list_conversations(user_id: str = None, limit: int = 20, offset: int = 0):
    """获取对话列表"""
    pass

@router.post("")
async def create_conversation(user_id: str = None, title: str = None):
    """创建新对话"""
    pass

@router.get("/{conversation_id}")
async def get_conversation(conversation_id: str, include_messages: bool = True):
    """获取对话详情（含消息列表）"""
    pass

@router.put("/{conversation_id}")
async def update_conversation(conversation_id: str, title: str):
    """更新对话标题"""
    pass

@router.delete("/{conversation_id}")
async def delete_conversation(conversation_id: str):
    """删除对话（级联删除消息）"""
    pass

@router.get("/{conversation_id}/messages")
async def get_messages(conversation_id: str, limit: int = 50, offset: int = 0):
    """获取对话的消息列表"""
    pass
```

---

## 五、前端实现要点

### 5.1 对话页面

```vue
<!-- src/views/Chat.vue -->
<template>
  <div class="chat-container">
    <div class="message-list" ref="messageList">
      <ChatMessage 
        v-for="msg in messages" 
        :key="msg.id"
        :message="msg"
      />
    </div>
    
    <div class="input-area">
      <el-input
        v-model="inputText"
        placeholder="请输入您的问题..."
        @keyup.enter="sendMessage"
      />
      <el-button type="primary" @click="sendMessage">发送</el-button>
    </div>
  </div>
</template>

<script setup>
import { ref } from 'vue'
import { chatStream } from '@/api'

const messages = ref([])
const inputText = ref('')

async function sendMessage() {
  if (!inputText.value.trim()) return
  
  // 添加用户消息
  messages.value.push({
    id: Date.now(),
    role: 'user',
    content: inputText.value
  })
  
  const userInput = inputText.value
  inputText.value = ''
  
  // 流式接收回复
  const eventSource = chatStream(userInput)
  
  let currentRole = null
  let currentContent = ''
  
  eventSource.onmessage = (event) => {
    const data = JSON.parse(event.data)
    
    if (data.type === 'role_start') {
      // 新角色开始
      currentRole = data
      messages.value.push({
        id: Date.now(),
        role: 'assistant',
        agentRole: data,
        content: ''
      })
    } else if (data.type === 'role_result') {
      // 更新角色内容
      const lastMsg = messages.value[messages.value.length - 1]
      lastMsg.content = data.content
    } else if (data.type === 'done') {
      eventSource.close()
    }
  }
}
</script>
```

### 5.2 消息组件

```vue
<!-- src/components/ChatMessage.vue -->
<template>
  <div :class="['message', message.role]">
    <div v-if="message.agentRole" class="role-tag">
      <span class="icon">{{ message.agentRole.icon }}</span>
      <span class="name">{{ message.agentRole.name }}</span>
    </div>
    <div class="content" v-html="renderMarkdown(message.content)"></div>
  </div>
</template>
```

---

## 六、部署方案

### 6.1 Docker Compose

```yaml
# docker-compose.yml
version: '3.8'

services:
  backend:
    build: ./backend
    ports:
      - "8000:8000"
    environment:
      - DEEPSEEK_API_KEY=${DEEPSEEK_API_KEY}
      - DATABASE_URL=postgresql://admin:password@postgres:5432/house_assistant
      - REDIS_URL=redis://redis:6379/0
    volumes:
      - ./data:/app/data
      - ./chroma_db:/app/chroma_db
    depends_on:
      - postgres
      - redis
  
  frontend:
    build: ./frontend
    ports:
      - "3000:80"
    depends_on:
      - backend

  postgres:
    image: postgres:15
    environment:
      POSTGRES_DB: house_assistant
      POSTGRES_USER: admin
      POSTGRES_PASSWORD: password
    ports:
      - "5432:5432"
    volumes:
      - postgres_data:/var/lib/postgresql/data

  redis:
    image: redis:7-alpine
    ports:
      - "6379:6379"
    volumes:
      - redis_data:/data

volumes:
  postgres_data:
  redis_data:
```

### 6.2 环境变量

```bash
# .env
DEEPSEEK_API_KEY=your_api_key_here
DEEPSEEK_BASE_URL=https://api.deepseek.com
DATABASE_URL=postgresql://admin:password@localhost:5432/house_assistant
REDIS_URL=redis://localhost:6379/0
```

---

## 七、开发计划

| 阶段 | 任务 | 预计时间 |
|------|------|---------|
| **Phase 1** | 数据准备 | 2-3天 |
| | - 生成市场模拟数据 | |
| | - 编写知识库文档 | |
| | - 初始化向量库 | |
| **Phase 2** | 后端核心 | 5-7天 |
| | - LLM客户端封装 | |
| | - 工具实现（财务、市场、政策） | |
| | - Agent引擎 | |
| | - RAG检索 | |
| | - API接口 | |
| **Phase 3** | 前端开发 | 4-5天 |
| | - 对话页面 | |
| | - 计算器页面 | |
| | - 市场分析页面 | |
| **Phase 4** | 集成测试 | 2-3天 |
| | - 端到端测试 | |
| | - 性能优化 | |
| | - Bug修复 | |

**总计**: 约2-3周

---

## 八、依赖清单

### 后端 requirements.txt

```txt
fastapi==0.109.0
uvicorn==0.27.0
httpx==0.26.0
pydantic==2.5.3
python-dotenv==1.0.0
chromadb==0.4.22
sentence-transformers==2.2.2
sqlalchemy==2.0.25
asyncpg==0.29.0
redis==5.0.1
```

### 前端 package.json

```json
{
  "dependencies": {
    "vue": "^3.4.0",
    "vue-router": "^4.2.0",
    "axios": "^1.6.0",
    "element-plus": "^2.5.0",
    "echarts": "^5.4.0",
    "marked": "^11.0.0"
  },
  "devDependencies": {
    "vite": "^5.0.0",
    "@vitejs/plugin-vue": "^5.0.0"
  }
}
```
